...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        || empty($id_satuan)) 
    {
        ...
        $nama_sparepart = mysqli_real_escape_string($conn, $nama_sparepart);
        $harga_jual = mysqli_real_escape_string($conn, $harga_jual);
        $id_kategori = mysqli_real_escape_string($conn, $id_kategori);
        $id_merk = mysqli_real_escape_string($conn, $id_merk);
        $id_tipe = mysqli_real_escape_string($conn, $id_tipe);
        $stock = mysqli_real_escape_string($conn, $stock);
        $id_satuan = mysqli_real_escape_string($conn, $id_satuan);

        $query = "INSERT INTO sparepart (nama_sparepart, harga_jual, id_kategori, id_merk, id_tipe_sp, stock, id_satuan) 
        VALUES ('$nama_sparepart', '$harga_jual', '$id_kategori', '$id_merk', '$id_tipe', '$stock', '$id_satuan')";
        
        if (mysqli_query($conn, $query)) {
            $_SESSION['pesan_sukses'] = "Data sparepart berhasil ditambahkan!";
...
<head>
    ...
    <title>Tambah Sparepart | Bengkel Bengawan</title>
    ...
    <div class="content">
        <div class="form-container">
            ...
            <h2 style="margin-top : 0; color: #333; border-bottom: 1px solid #eee; padding-bottom: 15px; margin-bottom: 20px;">
                <i class="fas fa-tags" style="color: #d32f2f"></i> Tambah Sparepart
            </h2>
            ...
            <form action="" method="POST">
                <div class="form-group">
                    ...
                    <input type="text" name="nama_sparepart" class="form-control" placeholder="Input nama sparepart" required>
                </div>

                <div class="form-group">
                    <label>Stock</label>
                    <input type="number" name="stock" class="form-control" placeholder="Input stock" required>
                </div>

                <div class="form-group">
                    <label>Harga Jual</label>
                    <input type="number" name="harga_jual" class="form-control" placeholder="Input harga jual" required>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="id_merk" class="form-control" required>
                        ...
                        <?php while($row = mysqli_fetch_assoc($q_merk)): ?>
                            <option value="<?php echo $row['id_merk']; ?>"><?php echo htmlspecialchars($row['nama_merk']); ?></option>
                        <?php endwhile; ?>
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="id_tipe" class="form-control" required>
                        ...
                        <?php while($row = mysqli_fetch_assoc($q_tipe)): ?>
                            <option value="<?php echo $row['id_tipe_sp']; ?>"><?php echo htmlspecialchars($row['nama_tipe']); ?></option>
                        <?php endwhile; ?>
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="id_satuan" class="form-control" required>
                        ...
                        <?php while($row = mysqli_fetch_assoc($q_satuan)): ?>
                            <option value="<?php echo $row['id_satuan']; ?>"><?php echo htmlspecialchars($row['nama_satuan']) . ' (' . htmlspecialchars($row['singkatan']) . ')'; ?></option>
                        <?php endwhile; ?>
                    </select>
                </div>

                <button type="submit" class="btn btn-submit">Simpan Sparepart</button>
            </form>
        </div>
    </div>
