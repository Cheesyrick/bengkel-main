...
$current_dsp_query = $conn->query("SELECT id_sparepart FROM detail_sparepart WHERE id_permintaan = $id_permintaan");
$current_dsp_ids = [];
while ($r = $current_dsp_query->fetch_assoc()) {
    $current_dsp_ids[] = $r['id_sparepart'];
}
                ...
                <div class="form-group">
                    ...
                    <select name="id_sparepart[]" class="form-control" multiple style="height: 100px;">
                        <?php foreach($sp_data as $sp): ?>
                            <?php $is_selected = in_array($sp['id_sparepart'], $current_dsp_ids); ?>
                            <option value="<?= $sp['id_sparepart'] ?>" <?= $is_selected ? 'selected' : '' ?> <?= (!$is_selected && $sp['stock'] <= 0) ? 'disabled' : '' ?>>
